...
def bg_rethink():
    conn = r.connect(host="localhost", port=28016, db="test")
...
@app.route('/api/<id>')
def get_data(id):
    stmt = "SELECT userid, time, xacc FROM data WHERE userid=%s limit 25"
    response = session.execute(stmt, parameters=[int(id)])
    response_list = []
    for val in response:
        response_list.append(val)
    jsonresponse = [{"userid": x.userid, "time": x.time, "xacc": x.xacc} for x in response_list]
...
def add_numbers():
    ...
    stmt = "SELECT userid, time, xacc FROM data WHERE userid=%s limit %s"
